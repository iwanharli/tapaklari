<div class="tf-widget-about-us main-content">
    <div class="themeflat-container">
        <div class="tf-about-us">
            <div class="row">
                <div class="col-md-6 image-wraper">
                    <div class="media">
                        <div class="media-v1 wow fadeInLeft animated">
                            <img class="mask-media" src="{{ asset('portal/images/about/mask1.png') }}" alt="image">
                            <img class="shape-media" src="{{ asset('portal/images/about/graphic.png') }}"
                                alt="image">
                        </div>
                        <img src="{{ asset('portal/images/about/mask2.png') }}" alt="image"
                            class="image-gr wow fadeInRight animated">
                        <img src="{{ asset('portal/images/about/Intersect.png') }}" alt="image"
                            class="intersect-img">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="about-box">
                        <img src="{{ asset('portal/images/about/graphic-box.png') }}" alt="image shape">
                        <!-- header style v1 -->
                        <div class="title-box title-small-v2">
                            <span class="sub-title wow fadeInUp animated">Welcome to runclub!</span>
                            <h2 class="title-section wow fadeInUp animated">TapakLari - Your Ultimate Running Community
                            </h2>
                        </div><!-- header style v1 -->
                        <p class="post wow fadeInUp animated">
                            Welcome to our vibrant running community, where we organize exciting running events,
                            provide helpful running tutorials, and keep you informed with the latest running news.
                        </p>
                        <div class="line"></div>
                        <div class="about-button-group">
                            <button class="flat-button wow fadeInUp animated">Find out more</button>
                            <div class="infor-about">
                                <img src="{{ asset('portal/images/about/info.png') }}" alt="">
                                <div class="info">
                                    <div class="name wow fadeInUp animated">Iwan Harli</div>
                                    <div class="job wow fadeInUp animated">Founder TapakLari</div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
